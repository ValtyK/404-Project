...
void write_wav_header(FILE *file, int sample_rate, int num_channels, int bits_per_sample, double duration_sec) {
    ...
    unsigned int data_size = (int)(sample_rate * duration_sec) * block_align;
    ...
    write_little_endian(num_channels, 2, file);
    write_little_endian(sample_rate, 4, file);
    write_little_endian(byte_rate, 4, file);
    write_little_endian(block_align, 2, file);
    write_little_endian(bits_per_sample, 2, file);
